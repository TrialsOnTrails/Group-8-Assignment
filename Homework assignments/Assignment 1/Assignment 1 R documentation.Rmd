...
#decision tree second try, training specifically for whose output probably >3 and < 200
...
predictions <- PCA(train_set_zeroCOLremoved, final_test_set)


### PCA second try, using cross-validation########################################

train_set_zeroCOLremoved <- train_set[,apply(train_set, 2, function(x) var(x)!=0)]
cvresults <- PCAcv(train_set_zeroCOLremoved, 123, 5)


### PCA third try, identify potential outliers and train a extra model for those outliers ########
...
predictions <- PCA(train_set_zeroCOLremoved,final_test_set2plus)
