...
PCAcv <- function(train, seed, kf) {
  ...
  # Assuming 'train' is your dataframe and 'outcome' is the name of your outcome variable
...
train_data <- train_set[split_index, ]  # 70% of data for training
#train_data$C281 <- ifelse(train_data$C281>0,1,0)
test_data <- train_set[-split_index, ]  # Remaining 30% for testing
#test_data$C281 <- ifelse(test_data$C281>0,1,0)
...
train_data <- train_set_0plus[split_index, ]  # 70% of data for training

test_data <- train_set_0plus[-split_index, ]  # Remaining 30% for testing
...
# identify 0 or 1 first
id.nonzero <- c(6,8,20,25,40,52,60,71,77,95,107,109,111,141,150,155,159,167,170,184,189,201,203)
id.nonzero <- id.nonzero+1

new_data_matrix <- as.matrix(final_test_set[id.nonzero,1:280])
predictions <- predict(model, new_data_matrix)
id.nonzero <- id.nonzero-1
predictions <- cbind(id.nonzero, predictions)
...
train_set <- read_csv("data/Processed Data Set/blogData_train duplicate removed.csv")
#train_set <- train_set[which(train_set$C281 > 0),]


#train_set$C62 <- log(1/mean(train_set$C62)*train_set$C62+1)

#train_set$C281 <- log(train_set$C281+1)

#colleft <- c(51:277,281)

#train_set <- train_set[,colleft]
...
test_data <- train_set[-split_index, ] 

#dataframe with 280 features and 1 output variable

input_data_train <- as.matrix(train_data[, !names(train_data) %in% "C281"])  # Selecting the first 280 columns as features
output_data_train <- as.matrix(train_data[, names(train_data) %in% "C281"]) # Assuming the output variable is in the 281st column
input_data_test <- as.matrix(test_data[, !names(test_data) %in% "C281"])
output_data_test <- as.matrix(test_data[, names(test_data) %in% "C281"])



model <- keras_model_sequential() %>%
  layer_dense(units = 50, activation = 'relu',input_shape = c(280)) %>%
  layer_dense(units = 180, activation = 'relu') %>%
  layer_dense(units = 180, activation = 'relu') %>%
  layer_dense(units = 1)
...
model %>% compile(
  ...
  optimizer = "adam",  
  metrics = 'mean_absolute_error'  # Accuracy can still be useful to see 
)
...
new_data_matrix <- as.matrix(test_set_201)
predictions <- predict(model, new_data_matrix)
id.nonzero <- id.nonzero-1
predictions <- cbind(id.nonzero, predictions)
...
#### 3.3.5.3 Neural network with PCA
```{r}
#### 3.3.5.2 Neural network using tensorflow and keras 
#train_set_5plus <- train_set[which(train_set$C281 > 5),]
#train_set_5plus <- train_set[,apply(train_set, 2, function(x) var(x)!=0)]
...
#### 3.3.5.4 Categorical neural network
```{r}

# transform output to classes
...
# Compile the model with Mean Squared Error as the loss function and an optimizer
model %>% compile(
  loss = 'categorical_crossentropy',  # Using MSE as the loss function
  optimizer = "adam",  
  metrics = 'accuracy'  # Accuracy can still be useful to see 
)
...
dtrain <- xgb.DMatrix(data = as.matrix(X_train), label = as.matrix(y_train$C281))
...
params <- list(
  ...
  max_depth = 6,               # can tune this and other parameters
  eta = 0.3,
  num_class = 438,     # number of classes
...
predictions <- predict(nb_model, test_set_201)
...
# Split the data into two features and target
X_train <- train_set[, -ncol(train_set)]
y_train <- train_set[, ncol(train_set)]

# Convert factors if there are categorical variables
# Assuming 'category_column' is a categorical feature that should be a factor
X_train$category_column <- as.factor(X_train$category_column)
y_train <- as.factor(y_train)


# Ensure y_train is a vector, not a data frame
if (is.data.frame(y_train)) {
  y_train <- unlist(y_train, use.names = FALSE)
}

# Check again if the number of rows in X_train matches the length of y_train
if (nrow(X_train) != length(y_train)) {
  stop(paste("Mismatch in the dataset: X_train has", nrow(X_train), "rows, but y_train has", length(y_train), "items."))
}

# If y_train is not a factor, convert it to factor for classification
...
RF_model <- randomForest(x = X_train, y = y_train, ntree = 281, mtry = sqrt(ncol(X_train)), importance = TRUE)
