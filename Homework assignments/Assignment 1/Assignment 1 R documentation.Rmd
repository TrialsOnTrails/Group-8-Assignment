...
# Assuming you have a dataframe `df` with 280 features and 1 output variable
input_data <- as.matrix(train_set[,1:280])  # Selecting the first 280 columns as features
output_data <- as.matrix(train_set[,281]) # Assuming the output variable is in the 281st column

# Normalize input data
input_data <- scale(input_data)

num_input_features <- 280
num_output_classes <- length(unique(output_data))
...
# Convert to factor in case it is not already
train_y_factor <- as.factor(train_y)

# Calculate the frequency of each class
table_y <- table(train_y_factor)

# Calculate class weights inversely proportional to class frequencies
class_weights <- max(table_y) / table_y

# Normalize the weights so that the smallest weight is 1.
class_weights_list <- list(0*class_weights / min(class_weights))



model <- keras_model_sequential() %>%
  layer_dense(units = 200, activation = 'relu', input_shape = c(280)) %>%
...
model %>% compile(
  ...
  optimizer = "adam",  # You can try different optimizers
  metrics = 'accuracy'  # Accuracy can still be useful to see even if it's not the objective
)
...
new_data_matrix <- as.matrix(test_set_201[,1:280])
predictions <- predict(model, new_data_matrix)
predictions

MSE(predictions, test_set_201)

predicted_classes <- predict_classes(model, new_data_matrix)
confusionMatrix(data = predicted_classes, reference = true_labels)
```
...
write.csv(as.vector(predictions), "csv_for_submission/031603_PCAplusDT.csv", row.names = FALSE)
