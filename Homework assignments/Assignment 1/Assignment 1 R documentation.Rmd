...
id <- c(8,20,24,25,30,40,71,95,109,111,131,141,155,167)
id <- id + 1
...
for(i in seq_along(folds)) {
    ...
    num_components <- which.max(prop_var_explained >= 0.5)
...
train_set_zeroCOLremovedPlus10 <- train_set[which(train_set$`281`>10),]
...
# Selecting the first few principal components based on the scree plot or variance explained
n_components <- 241 # for example, if you decided to keep the first two components
pca_scores <- pca_result$x[, 1:n_components]
...
#################################################################################
id <- c(8,20,24,25,30,40,49,71,95,109,111,131,141,155,167,184,203)
id <- id + 1
...
write.csv(outputcsv, "csv_for_submission/031407.csv", row.names = FALSE)
