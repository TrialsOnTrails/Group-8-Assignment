...
for (i in 1:49203){
  this.column <- ceiling(train_set[i,281] /5)
  print(this.column)
  # train_set[i,281+this.column] <- 1
  # for (j in 282:567) {
  #   if (j != 281+this.column){
  #     train_set[i,j] <- 0
  #   }
  # }
}
...
train_set$B2 <- ifelse(train_set$C281 > 10 & train_set$C281 < 15,1,0)set.seed(354)
train_set$B2 <- ifelse(train_set$C281 > 10 & train_set$C281 < 15,1,0)split_index <- sample(1:nrow(train_set), size = floor(0.8 * nrow(train_set)))
...
output_data_train <- as.matrix(train_data[, 282:292]) # Assuming the output variable is in the 281st column
input_data_test <- as.matrix(test_data[, 1:280])
output_data_test <- as.matrix(test_data[, 282:292])
...
write.csv(cbind(ID = (0:213), num_comments), "csv_for_submission/032103NeuralNet_categorical.csv", row.names = FALSE)
