...
model <- keras_model_sequential() %>%
  layer_dense(units = 180, activation = 'tanh',input_shape = c(280)) %>%
  layer_dense(units = 180, activation = 'tanh') %>%
  layer_dense(units = 180, activation = 'tanh') %>%
  layer_dense(units = 180, activation = 'tanh') %>%
  layer_dense(units = 180, activation = 'tanh') %>%
  layer_dense(units = 180, activation = 'tanh') %>%
...
#think about using 10 classes

train_set$C281 <-ifelse(train_set$C281<=10,"A",
                        ifelse(train_set$C281<=20,"B",
                               ifelse(train_set$C281<=30,"C",
                                      ifelse(train_set$C281<=40,"D",
                                             ifelse(train_set$C281<=50,"E",
                                                    ifelse(train_set$C281<=60,"F",
                                                           ifelse(train_set$C281<=70,"G",
                                                                  ifelse(train_set$C281<=80,"H",
                                                                         ifelse(train_set$C281<=90,"I",
                                                                                ifelse(train_set$C281<=100,"J","K"))))))))))
...
# Compile the model with Mean Squared Error as the loss function and an optimizer
model %>% compile(
  loss = 'categorical_cross_entropy',  # Using MSE as the loss function
  optimizer = "adamax",  
  metrics = 'accuracy'  # Accuracy can still be useful to see 
)
...
write.csv(cbind(ID = (0:22), num_comments = predictions), "csv_for_submission/031812_neuralne0plus.csv", row.names = FALSE)
